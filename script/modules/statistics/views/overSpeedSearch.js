...
var OverSpeedSearch = React.createClass({
    ...
    componentDidMount: function () {
        ...
        that.renderTree();
        that.renderSelect();
        $("input[name='start_date']").val(OverSpeedStore.data.listWeek[0]);
    ...
    handleChangeShowType: function (event) {
        ...
        switch(show_type){
            case "org":
                ...
                $('#org_tree_box').show();
                OverSpeedStore.trigger("showTableByOrg");
                break;
            case "car":
                $('.selectpicker').selectpicker('show');
                $('#org_tree_box').hide();
                OverSpeedStore.trigger("showTableByCar");
                break;
        }
    },
    handleChangeDate:function(){
        ...
        OverSpeedStore.data.listWeek = CommonFun.getDateArray("listWeek",end_date);
        OverSpeedStore.data.listMonth = CommonFun.getDateArray("listMonth",end_date);
        $("input[name='start_date']").val(OverSpeedStore.data.listWeek[0]);
        OverSpeedStore.data.isChangeDate = true;
    },
    /**
     * DOM操作回调，搜索按钮点击事件
    ...
    handleSearchClick:function (event) {
        ...
        if(OverSpeedStore.data.isChangeDate){
            var show_type = $(".search-box a.active[data-type]").data("type");
            switch(show_type){
                case "org":
                    OverSpeedStore.trigger("showTableByOrg");
                    OverSpeedStore.data.isChangeDate = false;
                    break;
                case "car":
                    OverSpeedAction.search();
                    break;
                default:
                    OverSpeedAction.search();
                    break;
    ...
    handleExportClick:function (event) {
        var tableData = $("#over_speed_table").bootstrapTable('getData') ;
...
export default OverSpeedSearch;