/**
 * @file 非规定时段用车统计-搜索框 Reflux View
...
var FoulTimeSearch = React.createClass({
    ...
    componentDidMount: function () {
        ...
        that.renderTree();
        that.renderSelect();
        $("input[name='start_date']").val(FoulTimeStore.data.listWeek[0]);
        DateTimePicker.setDate("input[name='end_date']",FoulTimeStore.data.listWeek[6]);
    ...
    handleChangeShowType: function (event) {
        ...
        switch(show_type){
            case "org":
                ...
                $('#org_tree_box').show();
                FoulTimeStore.trigger("showTableByOrg");
                break;
            case "car":
                $('.selectpicker').selectpicker('show');
                $('#org_tree_box').hide();
                FoulTimeStore.trigger("showTableByCar");
                break;
        }
    },
    handleChangeDate:function(){
        ...
        FoulTimeStore.data.listWeek = CommonFun.getDateArray("listWeek",end_date);
        FoulTimeStore.data.listMonth = CommonFun.getDateArray("listMonth",end_date);
        $("input[name='start_date']").val(FoulTimeStore.data.listWeek[0]);
        FoulTimeStore.data.isChangeDate = true;
    },
    /**
     * DOM操作回调，搜索按钮点击事件
    ...
    handleSearchClick:function (event) {
        ...
        if(FoulTimeStore.data.isChangeDate){
            var show_type = $(".search-box a.active[data-type]").data("type");
            switch(show_type){
                case "org":
                    FoulTimeStore.trigger("showTableByOrg");
                    FoulTimeStore.data.isChangeDate = false;
                    break;
                case "car":
                    FoulTimeAction.search();
                    break;
                default:
                    FoulTimeAction.search();
                    break;
    ...
    handleResetClick:function (event) {
        ...
        FoulTimeStore.data.listWeek = CommonFun.getDateArray("listWeek");
        DateTimePicker.setDate("input[name='start_date']",FoulTimeStore.data.listWeek[0]);
        DateTimePicker.setDate("input[name='end_date']",FoulTimeStore.data.listWeek[6]);
    },
    /**
     * DOM操作回调，导出按钮点击事件
    ...
    handleExportClick:function (event) {
        var tableData = $("#foul_time_table").bootstrapTable('getData');
