...
var OverSpeedChart = React.createClass({
    componentDidMount: function () {
        this.unsubscribe = OverSpeedStore.listen(this.listenEvent);
    },
    ...
    listenEvent: function(type,result){
        switch (type){
            case 'showOverSpeedChart':
                this.showOverSpeedChart(result);
                break;
        }
    },
    showOverSpeedChart:function(value,show_type){
        var series = this.initData(value,show_type);
        var categories = [];
        if(!!show_type && show_type == "month"){
            categories = OverSpeedStore.data.listMonth;
        }else {
            categories = OverSpeedStore.data.listWeek;
        }
        $('#over_speed_chart').highcharts({
            chart: {
                type: 'spline'
            },
            credits: {
    ...
    initData:function(oData,show_type){
        ...
        if(!!oData && oData.length >0) {
            ...
            oData.forEach(function(obj){
                ...
                if(!!show_type && show_type == "month"){
                    var listMonth = OverSpeedStore.data.listMonth;
                    listMonth.forEach(function(key){
                        series.data.push(parseInt(obj[key]));
                    });
                }else{
    ...
    handleChangeShowType: function (event) {
        ...
        tableData = $('#over_speed_table').bootstrapTable('getData', true) || [];
        tableData.forEach(function(item,index){
            org_ids.push(item.org_id);
        });
        var params = {
            org_ids:org_ids,
            start_time:OverSpeedStore.data.listWeek[0],
            end_time:OverSpeedStore.data.listWeek[6]
        };
        switch(show_type){
            case "week":
                break;
            case "month":
                params.start_time = OverSpeedStore.data.listMonth[0];
                params.end_time = OverSpeedStore.data.listMonth[OverSpeedStore.data.listMonth.length -1];
                break;
        }
        Urls.get(Urls.overSpeedStatistics,params,function(data){
            if(data.responseCode=="1" && data.responseMsg=="success"){
                that.showOverSpeedChart(data.datas,show_type);
            }else{
            ...
            <div className="row mart12 marb12">
                <div className="col-md-12">
                    <div className="chart-box-l">
                        <div  className="title">
                            <img src={__uri("/static/images/line-title.jpg")}/>超速统计
