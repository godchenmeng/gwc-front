...
var NotReturnSearch = React.createClass({
    ...
    componentDidMount: function () {
        ...
        that.renderTree();
        that.renderSelect();
        $("input[name='start_date']").val(NotReturnStore.data.listWeek[0]);
        DateTimePicker.setDate("input[name='end_date']",NotReturnStore.data.listWeek[6]);
    ...
    handleChangeShowType: function (event) {
        ...
        switch(show_type){
            case "org":
                ...
                $('#org_tree_box').show();
                NotReturnStore.trigger("showTableByOrg");
                break;
            case "car":
                $('.selectpicker').selectpicker('show');
                $('#org_tree_box').hide();
                NotReturnStore.trigger("showTableByCar");
                break;
        }
    },
    handleChangeDate:function(){
        ...
        NotReturnStore.data.listWeek = CommonFun.getDateArray("listWeek",end_date);
        NotReturnStore.data.listMonth = CommonFun.getDateArray("listMonth",end_date);
        $("input[name='start_date']").val(NotReturnStore.data.listWeek[0]);
        NotReturnStore.data.isChangeDate = true;
    },
    /**
     * DOM操作回调，搜索按钮点击事件
    ...
    handleSearchClick:function (event) {
        ...
        if(NotReturnStore.data.isChangeDate){
            var show_type = $(".search-box a.active[data-type]").data("type");
            switch(show_type){
                case "org":
                    NotReturnStore.trigger("showTableByOrg");
                    NotReturnStore.data.isChangeDate = false;
                    break;
                case "car":
                    NotReturnAction.search();
                    break;
                default:
                    NotReturnAction.search();
                    break;
    ...
    handleResetClick:function (event) {
        ...
        OverSpeedStore.data.listWeek = CommonFun.getDateArray("listWeek");
        DateTimePicker.setDate("input[name='start_date']",OverSpeedStore.data.listWeek[0]);
        DateTimePicker.setDate("input[name='end_date']",OverSpeedStore.data.listWeek[6]);
    },
    /**
     * DOM操作回调，导出按钮点击事件
     * @param {object} event 事件对象
    ...
    handleExportClick:function (event) {
        var tableData = $("#not_return_table").bootstrapTable('getData');
...
export default NotReturnSearch;