/**
 * @file 越界统计-搜索框 Reflux View
 * @author Banji 2017.09.05
 */
import React, { Component } from 'react';
import { render } from 'react-dom';

import OutsideAction from '../actions/outsideAction';
...
var OutsideSearch = React.createClass({
    ...
    componentDidMount: function () {
        ...
        that.renderTree();
        that.renderSelect();
        $("input[name='start_date']").val(OutsideStore.data.listWeek[0]);
        DateTimePicker.setDate("input[name='end_date']",OutsideStore.data.listWeek[6]);
    ...
    handleChangeShowType: function (event) {
        ...
        switch(show_type){
            case "org":
                ...
                $('#org_tree_box').show();
                OutsideStore.trigger("showTableByOrg");
                break;
            case "car":
                $('.selectpicker').selectpicker('show');
                $('#org_tree_box').hide();
                OutsideStore.trigger("showTableByCar");
                break;
        }
    },
    handleChangeDate:function(){
        ...
        OutsideStore.data.listWeek = CommonFun.getDateArray("listWeek",end_date);
        OutsideStore.data.listMonth = CommonFun.getDateArray("listMonth",end_date);
        $("input[name='start_date']").val(OutsideStore.data.listWeek[0]);
        OutsideStore.data.isChangeDate = true;
    },
    /**
     * DOM操作回调，搜索按钮点击事件
    ...
    handleSearchClick:function (event) {
        ...
        if(OutsideStore.data.isChangeDate){
            var show_type = $(".search-box a.active[data-type]").data("type");
            switch(show_type){
                case "org":
                    OutsideStore.trigger("showTableByOrg");
                    OutsideStore.data.isChangeDate = false;
                    break;
                case "car":
                    OutsideAction.search();
                    break;
                default:
                    OutsideAction.search();
                    break;
    ...
    handleResetClick:function (event) {
        ...
        OutsideStore.data.listWeek = CommonFun.getDateArray("listWeek");
        DateTimePicker.setDate("input[name='start_date']",OutsideStore.data.listWeek[0]);
        DateTimePicker.setDate("input[name='end_date']",OutsideStore.data.listWeek[6]);
    },
    /**
     * DOM操作回调，导出按钮点击事件
    ...
    handleExportClick:function (event) {
        var tableData = $("#outside_table").bootstrapTable('getData');
