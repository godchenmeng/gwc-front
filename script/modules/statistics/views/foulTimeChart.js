...
var FoulTimeChart = React.createClass({
    componentDidMount: function () {
        this.unsubscribe = FoulTimeStore.listen(this.listenEvent);
    },
    ...
    listenEvent: function(type,result){
        switch (type){
            case 'showFoulTimeChart':
                this.showFoulTimeChart(result);
                break;
        }
    },
    showFoulTimeChart:function(value,show_type){
        var series = this.initData(value,show_type);
        var categories = [];
        if(!!show_type && show_type == "month"){
            categories = FoulTimeStore.data.listMonth;
        }else {
            categories = FoulTimeStore.data.listWeek;
        }
        $('#foul_time_chart').highcharts({
            chart: {
                type: 'spline'
            },
            credits: {
    ...
    initData:function(oData,show_type){
        ...
        if(!!oData && oData.length >0) {
            ...
            oData.forEach(function(obj){
                ...
                if(!!show_type && show_type == "month"){
                    var listMonth = FoulTimeStore.data.listMonth;
    ...
    handleChangeShowType: function (event) {
        ...
        tableData = $('#foul_time_table').bootstrapTable('getData', true) || [];
        tableData.forEach(function(item,index){
            org_ids.push(item.org_id);
        });
        var params = {
            org_ids:org_ids,
            start_time:FoulTimeStore.data.listWeek[0],
            end_time:FoulTimeStore.data.listWeek[6]
        };
        switch(show_type){
            case "week":
                break;
            case "month":
                params.start_time = FoulTimeStore.data.listMonth[0];
                params.end_time = FoulTimeStore.data.listMonth[FoulTimeStore.data.listMonth.length -1];
                break;
        }
        Urls.get(Urls.foulTimeStatistics,params,function(data){
            if(data.responseCode == "1" && data.responseMsg=="success"){
                that.showFoulTimeChart(data.datas,show_type);
            }else{
            ...
            <div className="row mart12 marb12">
                <div className="col-md-12">
                    <div className="chart-box-l">
                        <div  className="title">
                            <img src={__uri("/static/images/line-title.jpg")}/>非规定时段用车统计
