/**
 * @file 违规停运统计-搜索框 Reflux View
 * @author Banji 2017.08.30
 */
import React, { Component } from 'react';
import { render } from 'react-dom';

import FoulParkAction from '../actions/foulParkAction'
...
var FoulParkSearch = React.createClass({
    ...
    componentDidMount: function () {
        ...
        that.renderTree();
        that.renderSelect();
        $("input[name='start_date']").val(FoulParkStore.data.listWeek[0]);
        DateTimePicker.setDate("input[name='end_date']",FoulParkStore.data.listWeek[6]);
    ...
    handleChangeShowType: function (event) {
        ...
        switch(show_type){
            case "org":
                ...
                $('#org_tree_box').show();
                FoulParkStore.trigger("showTableByOrg");
                break;
            case "car":
                $('.selectpicker').selectpicker('show');
                $('#org_tree_box').hide();
                FoulParkStore.trigger("showTableByCar");
                break;
        }
    },
    handleChangeDate:function(){
        ...
        FoulParkStore.data.listWeek = CommonFun.getDateArray("listWeek",end_date);
        FoulParkStore.data.listMonth = CommonFun.getDateArray("listMonth",end_date);
        $("input[name='start_date']").val(FoulParkStore.data.listWeek[0]);
        FoulParkStore.data.isChangeDate = true;
    },
    /**
     * DOM操作回调，搜索按钮点击事件
    ...
    handleSearchClick:function (event) {
        ...
        if(FoulParkStore.data.isChangeDate){
            var show_type = $(".search-box a.active[data-type]").data("type");
            switch(show_type){
                case "org":
                    FoulParkStore.trigger("showTableByOrg");
                    FoulParkStore.data.isChangeDate = false;
                    break;
                case "car":
                    FoulParkAction.search();
                    break;
                default:
                    FoulParkAction.search();
                    break;
    ...
    handleResetClick:function (event) {
        ...
        FoulParkStore.data.listWeek = CommonFun.getDateArray("listWeek");
        DateTimePicker.setDate("input[name='start_date']",FoulParkStore.data.listWeek[0]);
        DateTimePicker.setDate("input[name='end_date']",FoulParkStore.data.listWeek[6]);
    },
    /**
     * DOM操作回调，导出按钮点击事件
    ...
    handleExportClick:function (event) {
        var tableData = $("#foul_park_table").bootstrapTable('getData') ;
