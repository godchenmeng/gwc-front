...
var ViolationSearch = React.createClass({
    ...
    componentDidMount: function () {
        ...
        that.renderTree();
        that.renderSelect();
        $("input[name='start_date']").val(ViolationStore.data.listWeek[0]);
        DateTimePicker.setDate("input[name='end_date']",ViolationStore.data.listWeek[6]);
    ...
    handleChangeShowType: function (event) {
        ...
        switch(show_type){
            case "org":
                ...
                $('#org_tree_box').show();
                ViolationStore.trigger("showTableByOrg");
                break;
            case "car":
                $('.selectpicker').selectpicker('show');
                $('#org_tree_box').hide();
                ViolationStore.trigger("showTableByCar");
                break;
        }
    },
    handleChangeDate:function(){
        ...
        ViolationStore.data.listWeek = CommonFun.getDateArray("listWeek",end_date);
        ViolationStore.data.listMonth = CommonFun.getDateArray("listMonth",end_date);
        $("input[name='start_date']").val(ViolationStore.data.listWeek[0]);
        ViolationStore.data.isChangeDate = true;
    },
    /**
     * DOM操作回调，搜索按钮点击事件
    ...
    handleSearchClick:function (event) {
        ...
        if(ViolationStore.data.isChangeDate){
            var show_type = $(".search-box a.active[data-type]").data("type");
            switch(show_type){
                case "org":
                    ViolationStore.trigger("showTableByOrg");
                    ViolationStore.data.isChangeDate = false;
                    break;
                case "car":
                    ViolationAction.search();
                    break;
                default:
                    ViolationAction.search();
                    break;
    ...
    handleResetClick:function (event) {
        ...
        ViolationStore.data.listWeek = CommonFun.getDateArray("listWeek");
        DateTimePicker.setDate("input[name='start_date']",ViolationStore.data.listWeek[0]);
        DateTimePicker.setDate("input[name='end_date']",ViolationStore.data.listWeek[6]);
    },
    /**
     * DOM操作回调，导出按钮点击事件
    ...
    handleExportClick:function (event) {
        var tableData = $("#violation_table").bootstrapTable('getData');
