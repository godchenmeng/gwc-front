...
var FoulTaskSearch = React.createClass({
    ...
    componentDidMount: function () {
        ...
        that.renderTree();
        that.renderSelect();
        $("input[name='start_date']").val(FoulTaskStore.data.listWeek[0]);
        DateTimePicker.setDate("input[name='end_date']",FoulTaskStore.data.listWeek[6]);
    ...
    handleChangeShowType: function (event) {
        ...
        switch(show_type){
            case "org":
                ...
                $('#org_tree_box').show();
                FoulTaskStore.trigger("showTableByOrg");
                break;
            case "car":
                $('.selectpicker').selectpicker('show');
                $('#org_tree_box').hide();
                FoulTaskStore.trigger("showTableByCar");
                break;
        }
    },
    handleChangeDate:function(){
        ...
        FoulTaskStore.data.listWeek = CommonFun.getDateArray("listWeek",end_date);
        FoulTaskStore.data.listMonth = CommonFun.getDateArray("listMonth",end_date);
        $("input[name='start_date']").val(FoulTaskStore.data.listWeek[0]);
        FoulTaskStore.data.isChangeDate = true;
    },
    /**
     * DOM操作回调，搜索按钮点击事件
    ...
    handleSearchClick:function (event) {
        ...
        if(FoulTaskStore.data.isChangeDate){
            var show_type = $(".search-box a.active[data-type]").data("type");
            switch(show_type){
                case "org":
                    FoulTaskStore.trigger("showTableByOrg");
                    FoulTaskStore.data.isChangeDate = false;
                    break;
                case "car":
                    FoulTaskAction.search();
                    break;
                default:
                    FoulTaskAction.search();
                    break;
    ...
    handleResetClick:function (event) {
        ...
        FoulTaskStore.data.listWeek = CommonFun.getDateArray("listWeek");
        DateTimePicker.setDate("input[name='start_date']",FoulTaskStore.data.listWeek[0]);
        DateTimePicker.setDate("input[name='end_date']",FoulTaskStore.data.listWeek[6]);
    },
    /**
     * DOM操作回调，导出按钮点击事件
    ...
    handleExportClick:function (event) {
        var tableData = $("#foul_task_table").bootstrapTable('getData');
